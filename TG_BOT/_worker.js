          ...
          $(document).ready(function() {
            ...
            async function uploadFile(file) {
              try {
                ...
                const interfaceInfo = {
                  acceptTypes: 'image/gif,image/jpeg,image/jpg,image/png',
                  gifMaxSize: 20 * 1024 * 1024,
                  otherMaxSize: 20 * 1024 * 1024,
                  compressImage: true
                };
                const acceptedTypes = interfaceInfo.acceptTypes.split(',');
                if (!acceptedTypes.includes(file.type)) {
                  toastr.error('不支持的文件类型！请上传 GIF、JPEG、JPG 或 PNG 格式的文件。');
                  return;
                }
                if (file.type === 'image/gif') {
                  if (file.size > interfaceInfo.gifMaxSize) {
                    toastr.error('GIF 文件必须≤' + interfaceInfo.gifMaxSize / (1024 * 1024) + 'MB');
                    return;
                  }
                } else {
